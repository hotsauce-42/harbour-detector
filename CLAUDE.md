...
- Nearest-place is not the same question as "what is this harbour called". A port belongs to its city, and GeoNames will happily put an unrelated hamlet closer to the quay: Rostock's Überseehafen is 0.6 km from Petersdorf (population 0) and 7.5 km from Rostock; Kiel-Holtenau is 2.8 km from Knoop and 5.0 km from Kiel. `Gazetteer.nearest_seat()` plus the `phase4.port_city_*` keys handle it — when the chosen place has **no recorded population**, an administrative seat of ≥50k **in the same country** within 8 km takes over. Every clause is load-bearing: the population trigger spares Warnemünde (8,441), the seat requirement excludes a `PPL` carrying an implausible 53,443 next to Karrebæksminde, and the country clause stops Helsingør becoming Helsingborg across the Øresund. 8 km sits deliberately below the 10.8 km from the Warnemünde quays to Rostock. 28 of 328 harbours change; `port_city_max_km: 0` disables it.

- The port-city override is guarded by **municipality containment**, not distance alone. `Gazetteer.shares_municipality()` compares the finest GeoNames division code (`admin4` in Germany, `admin2` in Denmark) of the harbour's nearest place against the candidate city's. Measured over the 28 distance-picked overrides: 22 confirmed, 6 rejected — Hellerup×3 (Gentofte kommune, not København), Kastrup (Tårnby), Vallensbæk Strand (not Hvidovre), Sandwig (Glücksburg, not Flensburg). Every rejection was a correction. Containment is **only** a veto: as a *replacement* for the distance rule it was measurably worse — Nordic kommune seats are inland administrative towns, so it wanted Skovshoved→Charlottenlund, Vedbæk→Holte, Rantzausminde→Svendborg.

- `_AdminIndex` is built from the gazetteer table **before** `drop_non_settlements`, and `prepare_gazetteer.py` therefore writes unpopulated `PPLX` districts to the file even though nothing can be named after one. They are frequently the only row carrying the municipality code: Kiel-Holtenau reaches Kiel through the district "Holtenau", while the nearest surviving place, Knoop, is in another Kreis. Filtering them at prepare time silently drops the override from 22 to 19 — the failure is quiet, so `tests/test_gazetteer.py::test_a_municipality_is_resolved_through_a_dropped_district` pins it.

- `GAZETTEER_SCHEMA` gained `admin2`/`admin3`/`admin4` after the 115 MB file was already in use, so `from_parquet` reads only the columns the file actually has and pads the rest via `_with_missing_columns`. An older gazetteer loads unchanged; `has_municipalities` is then False and the veto stands down rather than rejecting everything. Do not "simplify" that back to `columns=GAZETTEER_SCHEMA.names` — it turns an upgrade into a 421 MB re-download.
...
- `st.tabs` **builds every tab's content on every run** — only visibility is client-side. So nothing whose construction has an effect may go inside one: no map, and above all no `Draw` control, since `st_folium` reports a single `all_drawings` and two live controls would leave no way to tell which geometry came back. That is why the map's mode selector (`View · Edit outline · Draw lock area`) sits above the tabs rather than being one.

- `manual_transit_like` is **tri-state and must stay so**. `None` means "nobody looked, the detector decides and keeps refreshing"; `False` means "a person checked; not a lock" and outranks the detector for good. Collapsing them into a bool would make un-flagging indistinguishable from never having looked, and the next run would simply flag it again. It replaces rather than merges — the opposite of `manual_outline_wkt`, which is a floor — because a person overruling a heuristic is the whole point. Phase 4's own verdict stays in `detected_transit_like`.

- Phase 5 matching is **one-to-one**: `_assign_ids` collects every cluster's best claim from `_rank_match` (which returns a comparable quality — an H3-overlap match outranks any centroid-only one), then gives each existing harbour to its single best claimant and mints fresh ids for the rest. Without that, two clusters inside one harbour's 500 m radius both take its id, and promoting that output to the existing DB bakes in a duplicate that re-matches both clusters forever. Do not "simplify" it back to a per-cluster loop.

- `harbour_id` is **not unique**, for two unrelated reasons, and both are visible in the reference output. (a) `make_harbour_id()` hashes `centroid_h3_r8`, and a res-8 cell is ~530 m edge / 0.74 km² — two separate harbours whose centroids land in the same cell get the *same* ID no matter how well matching behaves (Hvide Sande, 465 m apart; Cuxhaven, 631 m). Fixing that means a finer resolution in the ID, which re-IDs every harbour. (b) Phase 5 matching is **many-to-one**: nothing stops two clusters both matching one existing harbour, and `phase5.centroid_match_distance_meters` (500 m) is generous next to how close real harbours get (Hellerup, 442 m). Once such an output is promoted to the existing DB the duplicate is baked in and self-perpetuates. Strategy B takes the **nearest** candidate rather than the first (it was picking a 496 m neighbour over a 0 m exact match), and matching is now one-to-one — together those took the reference run from 4 duplicates to 2. The remaining 2 are res-8 collisions, which no matching change can fix.
